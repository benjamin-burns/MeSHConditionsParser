...
def parseFile(inputFile, out):
    '''
    Parses input JSON data given by {inputFile} and appends condition term mapping data 
    for all properly formatted conditions in {inputFile} to {out}.
        ...
        Ensures:
            The content of {out} is updated to contain all condition term mappings contained in 
    ...
    for term in data:
        row = {}
        row["term"] = term
        row["code"] = data[term]["mesh_code"]
        out.writerow(row)

def main():
    '''
    Main method for JSONMeSHTermParser.
    Outputs a csv file with mappings between MeSH terms and MeSH codes for conditions.
    ...
    OUTPUT_FILE_PATH = "termToCode.csv"
    FIELD_NAMES = ["term", "code"]
